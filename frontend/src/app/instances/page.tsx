...
import { ProviderSymbolPicker } from '@/components/provider-symbol-picker';
...
type ProviderInstrumentStatus = {
  symbols: string[];
  metadata: Record<string, Instrument>;
  loading: boolean;
  error: string | null;
};
...
const PROVIDER_SYMBOL_CACHE_PREFIX = 'instances:provider-symbols:v1:';

type CachedProviderCatalog = {
  symbols: string[];
  details?: Record<string, Instrument>;
};

const readCachedProviderCatalog = (providerName: string): CachedProviderCatalog | null => {
  if (typeof window === 'undefined') {
    return null;
  }
  try {
    const key = `${PROVIDER_SYMBOL_CACHE_PREFIX}${providerName.toLowerCase()}`;
    const raw = window.sessionStorage.getItem(key);
    if (!raw) {
      return null;
    }
    const parsed = JSON.parse(raw) as CachedProviderCatalog;
    if (!Array.isArray(parsed.symbols)) {
      return null;
    }
    const symbols = parsed.symbols.filter((symbol): symbol is string => typeof symbol === 'string');
    const details: Record<string, Instrument> = {};
    if (parsed.details && typeof parsed.details === 'object') {
      Object.entries(parsed.details).forEach(([key, value]) => {
        if (value && typeof value === 'object') {
          details[key] = value as Instrument;
        }
      });
    }
...
const writeCachedProviderCatalog = (providerName: string, catalog: CachedProviderCatalog) => {
  if (typeof window === 'undefined') {
    return;
  }
  try {
    const key = `${PROVIDER_SYMBOL_CACHE_PREFIX}${providerName.toLowerCase()}`;
    window.sessionStorage.setItem(key, JSON.stringify(catalog));
  } catch {
...
export default function InstancesPage() {
  ...
  const moduleStatusByName = useMemo(() => {
    ...
    modules.forEach((module) => {
      ...
      const pinnedHash = module.selectedRevisionHash || null;
      const latestHash = module.tagAliases?.latest ?? pinnedHash;
      let latestTag: string | null = module.selectedRevisionTag ?? null;
  ...
  const loadProviderInstrumentSymbols = useCallback(async (providerName: string) => {
    ...
    const cachedCatalog = readCachedProviderCatalog(providerName);
    setProviderInstrumentState((prev) => {
      ...
      const nextSymbols =
        cachedCatalog && cachedCatalog.symbols.length > 0 && (!existing || existing.symbols.length === 0)
          ? cachedCatalog.symbols
          : existing?.symbols ?? [];
      const nextMetadata = cachedCatalog?.details ?? existing?.metadata ?? {};
      return {
        ...prev,
        [providerName]: {
          symbols: nextSymbols,
    ...
    try {
      ...
      const metadata: Record<string, Instrument> = {};
      ...
      instruments.forEach((instrument) => {
        ...
        const normalizedSymbol = typeof instrument.symbol === 'string' ? instrument.symbol.trim().toUpperCase() : '';
        if (!normalizedSymbol) {
          return;
        }
      ...
      writeCachedProviderCatalog(providerName, { symbols, details: metadata });
      setProviderInstrumentState((prev) => ({
        ...prev,
        [providerName]: {
          symbols,
  ...
  const handleSymbolToggle = useCallback(
    (providerName: string, symbol: string, symbolChecked: boolean) => {
      let changed = false;
      setProviderSymbols((prev) => {
        ...
        return {
          ...prev,
          [providerName]: Array.from(current).sort((a, b) => a.localeCompare(b)),
        };
      });
      if (changed) {
        markGuidedDirty();
  ...
  const handleSelectAllSymbols = useCallback(
    (providerName: string, symbols: string[]) => {
      if (symbols.length === 0) {
        return;
      }
      ...
      setProviderSymbols((prev) => {
        ...
        if (
          current.length === uniqueSymbols.length &&
          current.every((value, index) => value === uniqueSymbols[index])
        ) {
          return prev;
        }
        changed = true;
  ...
  const handleClearSymbols = useCallback(
    (providerName: string) => {
      let changed = false;
      setProviderSymbols((prev) => {
        if (!prev[providerName] || prev[providerName].length === 0) {
          return prev;
        }
        changed = true;
        return {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const providerNames = Object.keys(payload.scope ?? {});
    ...
    <>
      ...
      <div className="flex items-center justify-between">
        ...
        >
          ...
          <SheetContent side="right" className="flex h-full flex-col sm:max-w-[56rem]">
            <SheetHeader>
              <SheetTitle>
    ...
    "symbols": ["BTC-USDT"]
                        ...
                        {selectedProviders.length > 0 && (
                          <div className="space-y-3">
                            {selectedProviders.map((providerName) => {
                              ...
                              const instrumentDetails = instrumentState?.metadata ?? {};
                              return (
                                <div key={providerName} className="space-y-2">
                                  <ProviderSymbolPicker
                                    providerName={providerName}
                                    symbols={availableSymbols}
                                    loading={instrumentState?.loading ?? false}
                                    error={instrumentState?.error ?? null}
                                    ...
                                    onToggleSymbol={(symbol, checked) => {
                                      setFormError(null);
                                      handleSymbolToggle(providerName, symbol, checked);
                                    }}
                                    onRetry={() => {
                                      setFormError(null);
                                      void loadProviderInstrumentSymbols(providerName);
                                    ...
                                    onSelectAll={(symbols) => {
                                      setFormError(null);
                                      handleSelectAllSymbols(providerName, symbols);
                                    }}
                                    onClearAll={() => {
                                      setFormError(null);
                                      handleClearSymbols(providerName);
                                    }}
                                  />
                                </div>
                              );
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {instances.map((instance) => {
          ...
          const usageLink = instance.links?.usage ?? null;
                      ...
                      <div className="flex flex-wrap items-center justify-between gap-3">
                        ...
                        {usageLink ? (
                          <Button variant="outline" size="sm" asChild>
                            {usageLink.startsWith('http') ? (
                              <a href={usageLink} target="_blank" rel="noreferrer">
                                View usage
                              </a>
                            ) : (
      ...
      >
        <SheetContent side="right" className="flex h-full flex-col sm:max-w-[64rem]">
          ...
          <ScrollArea className="flex-1 min-h-0 pr-1" allowYScroll allowXScroll={false}>
              ...
              <div className="space-y-4">
                <div className="flex flex-wrap items-center justify-between gap-2 text-xs text-muted-foreground">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    <Badge variant={historyDialogInstance.running ? 'success' : 'secondary'}>
                      {historyDialogInstance.running ? 'Running' : 'Stopped'}
                    </Badge>
                    {historyDialogInstance.links?.self ? (
                      <Button variant="ghost" size="sm" className="h-7 px-2" asChild>
                        {historyDialogInstance.links.self.startsWith('http') ? (
                          <a href={historyDialogInstance.links.self} target="_blank" rel="noreferrer">
                            Open details
                          </a>
                        ) : (
                          <Link href={historyDialogInstance.links.self}>Open details</Link>
                        )}
                      </Button>
                    ) : null}
                  </div>
