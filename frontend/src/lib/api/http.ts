...
export function createHttpClient(config: HttpClientConfig = {}): HttpClient {
  ...
  async function sendRequest(options: BaseRequestOptions & { body?: unknown }): Promise<Response> {
    ...
    try {
      ...
      throw normalizeFetchError(error, resolvedSignal);
    } finally {
      cleanup();
    }
  }
...
function parseErrorPayload(payload: unknown): StrategyErrorResponse | null {
  ...
  return {
    ...
    message,
    diagnostics,
  } satisfies StrategyErrorResponse;
}
...
function normalizeFetchError(error: unknown, signal?: AbortSignal): ApiError {
  if (error instanceof ApiError) {
    return error;
  }

  if (isAbortError(error)) {
    const reason = extractAbortReason(signal);
    const message = reason ?? 'Request was cancelled';
    return new ApiError(message, { cause: error });
  }

  if (error instanceof Error) {
    return new ApiError(error.message || 'Network request failed', { cause: error });
  }

  return new ApiError('Network request failed', { cause: error });
}
