...
function orderHistoryFiltersKey(filters?: OrderHistoryFilters) {
  if (!filters) {
    return undefined;
  }
  const normalizedState = filters.state?.length
    ? [...filters.state].sort((a, b) => a.localeCompare(b))
    : undefined;
  return {
...
function executionHistoryFiltersKey(filters?: ExecutionHistoryFilters) {
  if (!filters) {
    return undefined;
  }
  return {
...
export function useInstanceQuery(id?: string, enabled = false) {
  ...
  return useQuery<InstanceSnapshotResponse>({
...
export function useInstanceOrdersQuery(
  ...
  return useQuery<OrderHistoryResponse>({
    queryKey: queryKeys.instanceOrders(key, orderHistoryFiltersKey(filters)),
    queryFn: () => {
      if (!id) {
        throw new Error('Instance identifier missing');
      }
...
export function useInstanceExecutionsQuery(
  ...
  return useQuery<ExecutionHistoryResponse>({
    queryKey: queryKeys.instanceExecutions(key, executionHistoryFiltersKey(filters)),
    queryFn: () => {
      if (!id) {
        throw new Error('Instance identifier missing');
      }
...
export function useCreateInstanceMutation() {
  ...
  return useMutation<InstanceSnapshotResponse, unknown, InstanceSpec>({
