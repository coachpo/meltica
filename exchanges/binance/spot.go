...
func (s spotAPI) PlaceOrder(ctx context.Context, req core.OrderRequest) (core.Order, error) {
	svc := s.x.tradingServiceFor(core.MarketSpot)
	if svc == nil {
...
func (s spotAPI) GetOrder(ctx context.Context, symbol, id, clientID string) (core.Order, error) {
	...
	req := routingrest.OrderQueryRequest{Symbol: symbol, OrderID: id, ClientOrderID: clientID}
...
func (s spotAPI) CancelOrder(ctx context.Context, symbol, id, clientID string) error {
	svc := s.x.tradingServiceFor(core.MarketSpot)
	if svc == nil {
		return internal.Invalid("trading service unavailable")
	}
	req := routingrest.OrderCancelRequest{Symbol: symbol, OrderID: id, ClientOrderID: clientID}
