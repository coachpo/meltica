...
## 1. Data Bus (`internal/bus/databus/memory.go`)

### Metrics

| Metric Name | Type | Unit | Description | Labels |
|------------|------|------|-------------|--------|
| `databus.events.published` | Counter | `{event}` | Total events published to the bus | environment, event_type, provider, symbol |
| `databus.subscribers` | UpDownCounter | `{subscriber}` | Current number of active subscribers | environment, event_type |
| `databus.delivery.errors` | Counter | `{error}` | Failed event deliveries to subscribers | environment, event_type, error_type, reason |
| `databus.fanout.size` | Histogram | `{subscriber}` | Number of subscribers per fanout event | environment, event_type, provider, symbol |

**Histogram Buckets (databus.fanout.size)**:
```
[1, 2, 5, 10, 20, 50, 100]
```
...
| `dispatcher.events.buffered` | UpDownCounter | `{event}` | Currently buffered events | environment, event_type, provider, symbol |
...
| `orderbook.snapshot.applied` | Counter | `{snapshot}` | Snapshots applied | environment, provider, symbol, event_type |
...
**Examples**:
- `databus.events.published` - Databus component, events published
...
| `dispatcher.publish_batch` | Dispatcher | Batch publication to databus |
| `databus.Publish` | Databus | Event publication with fanout |
| `databus.dispatch` | Databus | Event delivery to subscribers |
...
4. `databus.fanout.size` - Links large fanouts to traces
...
2. **grafana-provider-dashboard.json**
   ...
   - Processing latency
   - Buffering and databus throughput
