...
// OrderBook represents the current state of the OKX order book for a symbol.
...
// UpdateFromOKXDelta applies OKX incremental book updates to the order book.
func UpdateFromOKXDelta(orderBook *OrderBook, bids, asks []corews.DepthLevel, prevSeqID, seqID int64, updateTime time.Time) bool {
	success := true

	orderBook.WithWriteLock(func(ob *OrderBook) {
		if ob.LastUpdateID > 0 {
			if prevSeqID != ob.LastUpdateID {
				ob.Bids = make(map[string]*big.Rat)
				ob.Asks = make(map[string]*big.Rat)
				ob.LastUpdateID = 0
				success = false
